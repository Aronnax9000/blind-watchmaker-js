$.widget( "dawk.triay_completenessGenebox", $.dawk.monochrome_genebox, {
    _init : function() {
...
$.widget( "dawk.triay_spokesGenebox", $.dawk.monochrome_genebox, {
    _init : function() {
...
//        // console.log('update from canvas:  nGeneboxes ' + geneboxes.length + ' biomorph ' + biomorph);
        ...
        for(let i = 0; i < 9; i++) {
            ...
            genebox.gene1to9box("option", "value", biomorph.genes[i]);
            genebox.gene1to9box("option", "gradientValue", biomorph.dGenes[i]);
        ...
        genebox.segDistGenebox("option", "gradientValue", biomorph.dGenes[9]);
        genebox.segDistGenebox("refresh");
        genebox = geneboxes.eq(11);
        genebox.triay_completenessGenebox("option", "value", biomorph.symmetrical);
        genebox.triay_completenessGenebox("refresh");
        genebox = geneboxes.eq(12);
        genebox.triay_spokesGenebox("option", "value", biomorph.spokesGene);
        genebox.triay_spokesGenebox("refresh");
    ...
    _create : function(options) {
        ...
        this.element.append(geneBox);
        geneBox = $("<div></div>").triay_completenessGenebox({geneboxCollection: this});
        geneBox.triay_completenessGenebox("option", "geneboxCollection", this);
        geneBox.triay_completenessGenebox("option", "geneboxIndex", 12);
        this.element.append(geneBox);
        geneBox = $("<div></div>").triay_spokesGenebox({geneboxCollection: this});
        geneBox.triay_spokesGenebox("option", "geneboxCollection", this);
        geneBox.triay_spokesGenebox("option", "geneboxIndex", 13);
        this.element.append(geneBox);
